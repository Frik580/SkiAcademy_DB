...
describe.sequential.runIf(runsOnFirestoreEmulator)('course provisioning emulator e2e', () => {
  beforeAll(() => {
    if (getApps().length === 0) {
  ...
  it('A. provisions legacy course through full persistence chain and enrolls participant', async () => {
    ...
    const expectedInterval = resolveManifestDayInterval(
      manifest.days[0]!,
      manifest.timeZone
    ).interval;
    expect(
      compareCanonicalTimestamps(courseDays[0]!.interval.startsAt, expectedInterval.startsAt)
    ).toBe(0);
    expect(
      compareCanonicalTimestamps(courseDays[0]!.interval.endsAt, expectedInterval.endsAt)
    ).toBe(0);
  ...
  it('B. is idempotent on replay with same command identity', async () => {
    const commands = createCommands();
  ...
  it('C. rejects conflicting manifest for already provisioned course', async () => {
    const commands = createCommands();
    expect(
      (await commands.execute(applyEnvelope('idem-provision-emulator-conflict-a'))).status
    ).toBe('success');
  ...
  it('D. rejects a conflicting CourseDay without committing a partial Course', async () => {
    const commands = createCommands();
  ...
  it('E. exposes legacy-only course outside operational catalog and canonical course as eligible', async () => {
    const commands = createCommands();
  ...
  it('F. rejects a malformed pre-existing CourseDay without creating an incomplete Course', async () => {
    const commands = createCommands();
  ...
  it('G. rejects provision when roster instructor is missing from catalog', async () => {
    const commands = createCommands();
  ...
  it('H. rejects enrollment when capacity is exhausted', async () => {
    const commands = createCommands();
    ...
    expect(
      (
        await commands.execute(
          applyEnvelope('idem-provision-emulator-capacity', singleSeatManifest)
        )
      ).status
    ).toBe('success');
  ...
  it('I. replaces legacy course document inside a single transaction without leaving it absent', async () => {
    const commands = createCommands();
  ...
  it('J. retries the same atomic manifest after a blocker is removed', async () => {
    const commands = createCommands();
    await seedConflictingCourseDayStub();
    const partialManifest = twoDayManifest();
    ...
    expect(partial.course).toBeUndefined();
    expect(partial.courseDays).toHaveLength(0);
    ...
    expect(countsAfterResume.claims).toBe(countsBeforeResume.claims + 2);
  ...
  it('K. rejects an alternate manifest after conflict without creating either Course', async () => {
    const commands = createCommands();
    await seedConflictingCourseDayStub();
    const partialManifest = twoDayManifest();
    expect(
      (
        await commands.execute(
          applyEnvelope('idem-provision-emulator-partial-conflict-a', partialManifest)
        )
      ).status
    ).toBe('error');
