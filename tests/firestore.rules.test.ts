...
describe('admin issue inbox revision signal', () => {
  ...
  it('allows admin read and denies student, instructor, unauth read and all client writes', async () => {
    ...
    const instructorRef = doc(instructorDb, 'admin_runtime', 'admin_issue_inbox');
    const anonymousRef = doc(anonymousDb, 'admin_runtime', 'admin_issue_inbox');
...
describe('admin lesson bookings revision signal', () => {
  beforeEach(async () => {
    await seedData(async (context) => {
      const db = context.firestore();
      await setDoc(doc(db, 'users', USER_ID), userProfile(USER_ID, 'user@example.com'));
      ...
      await setDoc(doc(db, 'admin_runtime', 'admin_lesson_bookings'), {
        revision: 10,
        updatedAt: { seconds: 1, nanoseconds: 0 },
      });
    });
  ...
  it('allows admin read and denies student, instructor, unauth read and all client writes', async () => {
    ...
    const instructorDb = testEnv
      ...
      .firestore();
    const anonymousDb = testEnv.unauthenticatedContext().firestore();
    const adminRef = doc(adminDb, 'admin_runtime', 'admin_lesson_bookings');
    const userRef = doc(userDb, 'admin_runtime', 'admin_lesson_bookings');
    const instructorRef = doc(instructorDb, 'admin_runtime', 'admin_lesson_bookings');
    const anonymousRef = doc(anonymousDb, 'admin_runtime', 'admin_lesson_bookings');
...
describe('admin planner revision signal', () => {
  beforeEach(async () => {
    await seedData(async (context) => {
      const db = context.firestore();
      await setDoc(doc(db, 'users', USER_ID), userProfile(USER_ID, 'user@example.com'));
      ...
      await setDoc(doc(db, 'admin_runtime', 'admin_planner'), {
        revision: 10,
        updatedAt: { seconds: 1, nanoseconds: 0 },
      });
    });
  ...
  it('allows admin read and denies student, instructor, unauth read and all client writes', async () => {
    ...
    const instructorDb = testEnv
      ...
      .firestore();
    const anonymousDb = testEnv.unauthenticatedContext().firestore();
    const adminRef = doc(adminDb, 'admin_runtime', 'admin_planner');
    const userRef = doc(userDb, 'admin_runtime', 'admin_planner');
    const instructorRef = doc(instructorDb, 'admin_runtime', 'admin_planner');
    const anonymousRef = doc(anonymousDb, 'admin_runtime', 'admin_planner');
...
describe('admin finance revision signal', () => {
  beforeEach(async () => {
    await seedData(async (context) => {
      const db = context.firestore();
      await setDoc(doc(db, 'users', USER_ID), userProfile(USER_ID, 'user@example.com'));
      ...
      await setDoc(doc(db, 'admin_runtime', 'admin_finance'), {
        revision: 10,
        updatedAt: { seconds: 1, nanoseconds: 0 },
      });
    });
  ...
  it('allows admin read and denies student, instructor, unauth read and all client writes', async () => {
    ...
    const instructorDb = testEnv
      ...
      .firestore();
    const anonymousDb = testEnv.unauthenticatedContext().firestore();
    const adminRef = doc(adminDb, 'admin_runtime', 'admin_finance');
    const userRef = doc(userDb, 'admin_runtime', 'admin_finance');
    const instructorRef = doc(instructorDb, 'admin_runtime', 'admin_finance');
    const anonymousRef = doc(anonymousDb, 'admin_runtime', 'admin_finance');
...
describe('admin people revision signal', () => {
  beforeEach(async () => {
    await seedData(async (context) => {
      const db = context.firestore();
      await setDoc(doc(db, 'users', USER_ID), userProfile(USER_ID, 'user@example.com'));
      ...
      await setDoc(doc(db, 'admin_runtime', 'admin_people'), {
        revision: 10,
        updatedAt: { seconds: 1, nanoseconds: 0 },
      });
    });
  ...
  it('allows admin read and denies student, instructor, unauth read and all client writes', async () => {
    ...
    const instructorDb = testEnv
      ...
      .firestore();
    const anonymousDb = testEnv.unauthenticatedContext().firestore();
    const adminRef = doc(adminDb, 'admin_runtime', 'admin_people');
    const userRef = doc(userDb, 'admin_runtime', 'admin_people');
    const instructorRef = doc(instructorDb, 'admin_runtime', 'admin_people');
    const anonymousRef = doc(anonymousDb, 'admin_runtime', 'admin_people');
...
describe('admin booking change requests revision signal', () => {
  beforeEach(async () => {
    await seedData(async (context) => {
      const db = context.firestore();
      await setDoc(doc(db, 'users', USER_ID), userProfile(USER_ID, 'user@example.com'));
      ...
      await setDoc(doc(db, 'admin_runtime', 'admin_booking_change_requests'), {
        revision: 10,
        updatedAt: { seconds: 1, nanoseconds: 0 },
      });
    });
  ...
  it('allows admin read and denies student, instructor, unauth read and all client writes', async () => {
    ...
    const instructorDb = testEnv
      ...
      .firestore();
    const anonymousDb = testEnv.unauthenticatedContext().firestore();
    const adminRef = doc(adminDb, 'admin_runtime', 'admin_booking_change_requests');
    const userRef = doc(userDb, 'admin_runtime', 'admin_booking_change_requests');
    const instructorRef = doc(instructorDb, 'admin_runtime', 'admin_booking_change_requests');
    const anonymousRef = doc(anonymousDb, 'admin_runtime', 'admin_booking_change_requests');
...
describe('admin runtime unknown revision document', () => {
  beforeEach(async () => {
    await seedData(async (context) => {
      const db = context.firestore();
      await setDoc(doc(db, 'users', USER_ID), userProfile(USER_ID, 'user@example.com'));
      await setDoc(doc(db, 'admin_runtime', 'admin_unknown_signal'), {
        revision: 10,
        updatedAt: { seconds: 1, nanoseconds: 0 },
      });
    });
  ...
  it('denies admin and non-admin read and all client writes for an unknown documentId', async () => {
    const adminDb = testEnv
      .authenticatedContext(OWNER_ID, { email: 'owner@example.com' })
      .firestore();
    const userDb = testEnv.authenticatedContext(USER_ID, { email: 'user@example.com' }).firestore();
    const anonymousDb = testEnv.unauthenticatedContext().firestore();
    const adminRef = doc(adminDb, 'admin_runtime', 'admin_unknown_signal');
    const userRef = doc(userDb, 'admin_runtime', 'admin_unknown_signal');
    const anonymousRef = doc(anonymousDb, 'admin_runtime', 'admin_unknown_signal');
