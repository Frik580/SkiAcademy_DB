...
export const ClientsManager: React.FC<ClientsManagerProps> = ({
  ...
  const handleClientSubmit = async (e: React.FormEvent) => {
    ...
    const finalInstructorId = isNowInstructor ? oldInstructorId || `ins_${uId}` : '';
      ...
      if (isNowInstructor && !wasInstructor) {
        ...
        if (!exists) {
          const newIns: Instructor = {
            ...
            specialty: 'both',
            rating: editingClient?.instructorId
              ? instructors.find((i) => i.id === editingClient.instructorId)?.rating || 0
              : 0,
            reviewsCount: 0,
          ...
          if (existingIns && !existingIns.isAvailable) {
  ...
  return (
    <div className="space-y-4 transition-colors duration-300 w-full min-w-0 overflow-hidden">
      ...
      <div className="grid lg:grid-cols-12 gap-6 w-full min-w-0 overflow-hidden">
        ...
        <div
          className={`${showClientAddForm ? 'lg:col-span-8' : 'lg:col-span-12'} space-y-4 transition-all duration-300 w-full min-w-0 overflow-hidden`}
        >
          {/* Search Box */}
