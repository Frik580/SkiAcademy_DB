...
Для быстрого тестирования функциональности создайте тестовых пользователей одним из способов:

- **Через интерфейс приложения**: зарегистрируйте нового клиента через форму регистрации. Новому пользователю автоматически начисляется стартовый баланс $250.
- **Через Firebase Console**: создайте пользователя в разделе Authentication, затем добавьте документ в коллекцию `users/{uid}` с полями `uid`, `email`, `displayName` и `role: 'user'`. Чтобы назначить администратора, установите `role: 'admin'` и `systemRole: 'owner'` (последнее требует соответствующих прав в Firestore rules).

> **Не указывайте реальные пароли и учётные данные в документации или в коде репозитория.**
