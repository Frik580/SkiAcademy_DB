...
function seedParticipantRecord(
...
async function seedBase(
  ...
  if (options.includeParticipantB) {
    await firestore
      .doc(`participants/${participantIdB}`)
      .set(seedParticipantRecord(participantIdB, managementIdB));
    await firestore
      .doc(`participant_management/${managementIdB}`)
      .set(seedManagementRecord(managementIdB, participantIdB));
  }
...
describe.sequential.runIf(runsOnFirestoreEmulator)(
  ...
  () => {
    ...
    it('A. >=7d client cancellation refunds fully, releases seat, and credits wallet', async () => {
    ...
    it('B. >=2d <7d 50% cancellation uses floor rounding on odd retained amount', async () => {
      await clearCollections(firestore);
      ...
      expect(state.walletBalance).toBe(
        ODD_COURSE_PRICE_KZT * 2 - ODD_COURSE_PRICE_KZT + expectedRefund
      );
      expect(state.availableSeats).toBe(8);
      expect(state.refundEvents).toBe(1);
    }, 30_000);

    it('C. <2d cancellation enters pending_cancellation without refund or resource release', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
      ...
      expect(ownedClaims.some((claim) => claim?.claimKind === 'course_seat_pre_start')).toBe(true);
      expect(
    ...
    it('D. withdrawing pending cancellation restores confirmed without resource churn', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('E. admin approves pending cancellation before start with partial refund', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('F. admin rejects pending cancellation and restores confirmed', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('G. post-start zero-refund approval withdraws enrollment without increasing capacity', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('H. post-start positive-refund approval cancels enrollment without increasing capacity', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('I. pre-start transfer succeeds with same enrollmentId and paymentId', async () => {
      await clearCollections(firestore);
    ...
    it('J. transfer fails on booking conflict and leaves source enrollment unchanged', async () => {
      await clearCollections(firestore);
    ...
    it('N. re-enrollment after pre-start cancel creates a new enrollmentId', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      ...
      expect(state.availableSeats).toBe(7);
      expect((await readEnrollment(second.enrollmentId))?.lifecycle.status).toBe('confirmed');
    }, 30_000);

    it('I-up. pre-start transfer reprices payment upward and debits wallet for delta', async () => {
      await clearCollections(firestore);
    ...
    it('I-down. pre-start transfer reprices payment downward and refunds wallet delta', async () => {
      await clearCollections(firestore);
    ...
    it('K. transfer vs last-seat enrollment race leaves exactly one winner', async () => {
      await clearCollections(firestore);
      ...
      const successes = [transferResult, enrollResult].filter(
    ...
    it('L. duplicate transfer races serialize to one target course', async () => {
      await clearCollections(firestore);
      await seedBase(WALLET_START_KZT, 8, {
        includeAdminAccount: true,
        includeCourseB: true,
    ...
    it('M. transfer vs cancellation race serializes without double finance or seat churn', async () => {
      await clearCollections(firestore);
      await seedBase(WALLET_START_KZT, 8, {
        includeAdminAccount: true,
        includeCourseB: true,
      ...
      const successes = [transferResult, cancelResult].filter(
        (result) => result.status === 'success'
      );
      expect(successes).toHaveLength(1);
    ...
    it('O. guest expiry before course start releases seat and claims without wallet', async () => {
      await clearCollections(firestore);
      ...
      if (expireResult.status === 'error') {
        throw new Error(
          `expire failed: ${expireResult.error.code} ${JSON.stringify(expireResult.error.details)}`
        );
      }
    ...
    it('P. guest expiry at course start terminalizes without increasing availableSeats', async () => {
      await clearCollections(firestore);
      await seedBase(0, 8, { includeGuestParticipant: true });
    ...
    it('Q. replays successful cancellation without duplicate refund or seat release', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('R. stale revision races leave exactly one durable lifecycle winner', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
      ...
      const successes = [approveResult, rejectResult].filter(
        (result) => result.status === 'success'
      );
    ...
    it('T. omits undefined optional lifecycle fields without Firestore write errors', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('direct_cancel pre-start zero refund withdraws account enrollment and releases seat', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
    ...
    it('direct_cancel post-start zero refund withdraws without increasing capacity', async () => {
      const createCommandsAt = createCommands('2026-01-01T00:00:00.000Z');
      const funded = await createFundedEnrollment(createCommandsAt, {
