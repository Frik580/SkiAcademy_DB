...
Known resort slide logical image keys (`wall` through `wall7`, `about`, and `random`) are preserved as-is and do not create media-copy entries. The keys are defined in the shared-domain package; the frontend resolves them to the same Carve Object Storage assets in staging and production. Other public `https://storage.yandexcloud.net/carve/...` URLs are preserved without query strings or fragments, except Course, Instructor, and image-cache paths. Firebase binaries are eligible only for flat `banners/{filename}.{png|jpg|jpeg|webp}` references in resort slides. The source bucket, object path, content hash, and content type are recorded. Firebase download tokens are never put in the manifest. During apply, bytes are re-read and hash-checked, then written with a Storage generation precondition to a content-addressed `promotion-assets/config/` path. The slide reference is rewritten to that production object. Unknown logical keys, arbitrary external URLs, and other Storage paths fail closed with an export issue.
