...
- a Booking party contains one or more unique Participants; canonical lesson settings define the current business maximum for new creation and additions;
...
- `P` be the actual Participants mutated by the command;
...
| Illustrative eight-Participant Booking | one Booking, one Instructor claim, eight Participant claims, up to `sum(B(i))` guard mutations, Participant authorization/block checks, Payment/Wallet, idempotency, Activity Log, and required outbox obligations                                             | commonly 35–80 mutations and 40–100 reads                                                                                                      |
