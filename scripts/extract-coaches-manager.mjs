...
const dragOverEndIdx = adminLines.findIndex(
  (l, i) => i > showAddIdx && l.includes('const [isDragOver, setIsDragOver]')
);
...
const processImageEnd = adminLines.findIndex(
  (l, i) =>
    i > fileChangeIdx && l.trim() === '};' && adminLines[i + 1]?.includes('User/Admin Management')
);
...
const handleDeleteEnd = adminLines.findIndex(
  (l, i) => i > handleSubmitIdx && l.includes('const handleDeleteCoach')
);
...
const gridStart = adminLines.findIndex(
  (l) => l.includes('grid lg:grid-cols-12') && l.includes('overflow-hidden')
);
