...
function plannerWindow(
  ...
  const days = windowDays ?? (view === 'week' ? 7 : 1);
...
function projectAdminPlannerInstructorPresentation(
  record: NonNullable<ReturnType<typeof parseInstructorCatalog>>,
  revision: ReturnType<typeof parseInstructorCatalogRevision>
) {
  const base = {
...
export async function queryAdminPlannerReadModels(
  ...
  const window = plannerWindow(input.localDate, input.view, input.timeZone, input.windowDays);
  const rangeStart = Math.max(0, window.startsAt.seconds - PLANNER_OCCUPANCY_LOOKBACK_SECONDS);
  const rangeEnd = window.endsAt.seconds;

  const [instructorSnap, bookingPage, blockPage, dayPage] = await Promise.all([
    firestore.collection('instructors').limit(64).get(),
    paginateWindowQuery(
      firestore
        .collection('bookings')
  ...
  const instructors = instructorSnap.docs
    .map((document) => {
      ...
      return projectAdminPlannerInstructorPresentation(
        record,
        parseInstructorCatalogRevision(document.data() as Record<string, unknown>)
      );
    })
  ...
  const bookings = bookingPage.docs
    ...
    .filter(
      (booking) =>
        ...
        intervalsOverlap(booking.occurrence.interval, window)
    );
  ...
  for (const document of blockPage.docs) {
    ...
    if (!intervalsOverlap(block.interval, window)) continue;
  ...
  for (const document of dayPage.docs) {
    ...
    if (!day) continue;
    if (!intervalsOverlap(day.interval, window)) continue;
