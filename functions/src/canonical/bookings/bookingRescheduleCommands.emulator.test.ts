...
function claimsForBookingOccurrence(
  ...
  return claims.docs.filter(
    (doc) => doc.data().ownerId === bookingIdValue && doc.data().occurrenceId === occurrenceIdValue
  );
}
...
describe.skipIf(!runsOnFirestoreEmulator)('booking reschedule Firestore emulator', () => {
  ...
  it('A. serializes concurrent client self-service reschedules to one winner with single occurrence rotation', async () => {
  ...
  it('A-claim. successful same-resource reschedule releases old claims and activates new occurrence claims', async () => {
    const commands = createCommands('2026-01-01T00:00:00.000Z');
    await createConfirmedBooking(commands);
    const bookingBefore = (await firestore.doc(`bookings/${bookingId}`).get()).data();
    const startsAt = bookingBefore?.occurrence.interval.startsAt;
  ...
  it('B. rejects reschedule when target instructor interval is already claimed and preserves original booking state', async () => {
    const commands = createCommands('2026-01-01T00:00:00.000Z');
    await createConfirmedBooking(commands);
    ...
    expect(oldOccurrenceClaims.every((doc) => doc.data().lifecycle?.status === 'active')).toBe(
      true
    );
  ...
  it('C. rejects reschedule when target participant interval is already claimed and preserves original booking state', async () => {
    const commands = createCommands('2026-01-01T00:00:00.000Z');
    await createConfirmedBooking(commands);
  ...
  it('D. admin instructor change reprices atomically with claim rotation and preserved payment identity', async () => {
    const commands = createCommands('2026-01-01T00:00:00.000Z');
    await createConfirmedBooking(commands);
    const walletBefore = (await firestore.doc(`users/${accountId}/wallet/state`).get()).data();
    ...
    expect(newOccurrenceClaims.every((doc) => doc.data().lifecycle?.status === 'active')).toBe(
      true
    );
    expect(newOccurrenceClaims.some((doc) => doc.data().resourceId === instructorTwoId)).toBe(true);
    expect(newOccurrenceClaims.some((doc) => doc.data().resourceId === participantId)).toBe(true);
  ...
  it('E. rolls back failed admin instructor change on target conflict without partial durable state', async () => {
    const commands = createCommands('2026-01-01T00:00:00.000Z');
    await createConfirmedBooking(commands);
    ...
    expect(oldOccurrenceClaimsAfter.every((doc) => doc.data().lifecycle?.status === 'active')).toBe(
      true
    );
  ...
  it('F. replays successful reschedule and admin service change without duplicate mutation', async () => {
    const commands = createCommands('2026-01-01T00:00:00.000Z');
    await createConfirmedBooking(commands);
    const bookingBefore = (await firestore.doc(`bookings/${bookingId}`).get()).data();
    const startsAt = bookingBefore?.occurrence.interval.startsAt;
    ...
    const occurrenceAfterReschedule = (await firestore.doc(`bookings/${bookingId}`).get()).data()
    ...
    const occurrenceAfterInstructor = (await firestore.doc(`bookings/${bookingId}`).get()).data()
      ?.occurrence.occurrenceId;
  ...
  it('F-attendance. occurrence rotation does not rewrite existing attendance on the old occurrence', async () => {
    const commands = createCommands('2026-01-01T00:00:00.000Z');
    await createConfirmedBooking(commands);
  ...
  it('G. commits reschedule through real Firestore without undefined-field write failures when optional instructor fields are absent', async () => {
    await firestore.doc(`instructors/${instructorId}`).set({
      id: instructorId,
      name: 'Emulator Coach',
      pricePerHour: 120,
