...
describe.skipIf(!runsOnFirestoreEmulator)('finance correction commands emulator', () => {
  ...
  it('A. reconciliation detects mismatch without financial mutation', async () => {
  ...
  it('B. reconciliation replay and concurrent dedupe create one AdminIssue', async () => {
    const commands = createCommands();
  ...
  it('C. admin correction commits payment, wallet, event, audit, and idempotency atomically', async () => {
  ...
  it('D. correction replay does not duplicate payment, wallet, event, or audit', async () => {
    await seedCorrectionFixture();
    const commands = createCommands();
  ...
  it('E. correction vs provider payment event serializes without double settlement', async () => {
    await seedCorrectionFixture();
    await firestore
      .collection('payments')
      .doc(paymentId)
  ...
  it('G. correction vs price adjustment serializes without lost payment revision', async () => {
    await seedCorrectionFixture();
    const commands = createCommands();
  ...
  it('H. incremental requirement inconsistency opens deterministic AdminIssue without party mutation', async () => {
    await seedCorrectionFixture();
  ...
  it('J. historical monetary events remain unchanged after compensating correction', async () => {
    await seedCorrectionFixture();
  ...
  it('K. illegal correction beyond retained leaves durable state unchanged', async () => {
    await seedCorrectionFixture();
    const commands = createCommands();
    const paymentBefore = (await firestore.collection('payments').doc(paymentId).get()).data();
  ...
  it('L. stale payment revision rejects second correction based on old revision', async () => {
    await seedCorrectionFixture();
    const commands = createCommands();
  ...
  it('N. correction with absent optional metadata serializes without undefined write errors', async () => {
    await seedCorrectionFixture();
    const commands = createCommands();
    const envelope: CommandEnvelope<'record_financial_correction'> = {
      kind: 'record_financial_correction',
  ...
  it('F. correction vs cancellation refund serializes without impossible finance state', async () => {
    await clearCollections([
      'users',
      'participants',
    ...
    await firestore
      ...
      .collection('wallet')
      .doc('state')
      .set(seedWallet(WALLET_START_KZT));
  ...
  it('I. attendance_payment_conflict correction resolves issue without mutating attendance', async () => {
    await clearCollections([
      'users',
      'participants',
      'participant_management',
    ...
    expect(
      (await firestore.collection('monetary_events').doc(correctionEventId).get()).exists
    ).toBe(true);
    expect(
      (
        await firestore
          .doc(`activity_logs/${activityLogIdFromCommandId(correctionIdentity.commandKey)}`)
          .get()
      ).exists
    ).toBe(true);
  }, 30_000);

  it('O. rebuild_payment_projection vs provider event serializes with consistent eventRevision', async () => {
    await seedCorrectionFixture();
